...
const List = () => {
  ...
  return (
    <Page className={classes.root} title="이벤트 관리">
      <Container maxWidth={false}>
        <Typography variant="h3" color="textPrimary">
          이벤트 관리
        </Typography>
        <EventContainer.Filter pagination={pagination} filter={filter} />
        <Box mt={3}>
          <Box className={classes.buttons} pt={1}>
            ...
            <Button
              variant="outlined"
              color="primary"
              onClick={() => history.push('/event/register')}
            >
              신규등록
            </Button>
          </Box>
          <EventContainer.Table pagination={pagination} filter={filter} />
        </Box>
      </Container>
    </Page>
  );
